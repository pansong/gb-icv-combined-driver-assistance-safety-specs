...
> a) 对于M₁类以及N₁类汽车不大于3 $\text{m/s}^2$；

> b) 对于M₂类、M₃类、N₂类以及N₃类汽车不大于2.5 $\text{m/s}^2$。
...
> a) 对于M₁类以及N₁类汽车，车辆横向加速度不大于3.3 $\text{m/s}^2$；

> b) 对于M₂类、M₃类、N₂类以及N₃类汽车，车辆横向加速度不大于2.8 $\text{m/s}^2$。

**4.6.1.7** 对于M₁类汽车，若符合以下全部要求，则允许车辆横向加速度大于3 $\text{m/s}^2$：
...
**4.6.2.3.2.8** 对于M₁类以及N₁类汽车，换道执行阶段车辆的最大横向加速度不应大于3.5 $\text{m/s}^2$，且横向加速度变化率在任意0.5 $\text{s}$内的平均值不应大于5 $\text{m/s}^3$。对于M₂类、M₃类、N₂类以及N₃类汽车，换道执行阶段车辆的最大横向加速度不应大于2.5 $\text{m/s}^2$，且横向加速度变化率在任意0.5 $\text{s}$内的平均值不应大于5 $\text{m/s}^3$。
